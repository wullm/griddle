...
int main(int argc, char *argv[]) {
    ...
    if (output_list[0] == a_begin) {
        if (pars.DoHaloFindingWithSnapshots) {
            ...
            analysis_fof(particles, boxlen, N, M, local_partnum, max_partnum, linking_length, pars.MinHaloParticleNum, /* output_num = */ 0, a_begin, &us, &pcs, &cosmo, &pars, a_begin);
        ...
        for (int j = 0; j < num_outputs; j++) {
            if (output_list[j] > a && output_list[j] <= a_next) {
                ...
                if (pars.DoHaloFindingWithSnapshots) {
                    ...
                    analysis_fof(particles, boxlen, N, M, local_partnum, max_partnum, linking_length, pars.MinHaloParticleNum, /* output_num = */ j, output_list[j], &us, &pcs, &cosmo, &pars, output_list[j]);
